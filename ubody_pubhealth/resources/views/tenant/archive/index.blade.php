        ...
        <div class="box-header">
            <form class="form-inline" method="get" action="{{ URL::action("Tenant\\ArchiveController@index") }}">
                <div class="form-group">
                    <a href="{{ URL::action("Tenant\\ArchiveController@create") }}" class="btn btn-primary">添加</a>
                </div>
            ...
            <table class="table table-hover table-striped table-bordered">
                ...
                @foreach($list as $model)
                    <tr>
                        ...
                        <td>
                            <a href="{{ URL::action('Tenant\ArchiveController@show', $model->id) }}">{{ $model->code }}</a>
                        </td>
                        ...
                        <td>{{ $model->contract ? ($model->contract->status == 2?'已签约':($model->contract->status==1?'预约':'已作废')) : '未签约' }}</td>
                        <td style="text-align: right">
                            <a href="{{ URL::action("Tenant\\ArchiveController@edit", $model->id) }}"
                               class="btn btn-success btn-xs btn-flat">编辑</a>
                            <a href="{{ URL::action("Tenant\\ArchiveController@destroy", $model->id) }}"
